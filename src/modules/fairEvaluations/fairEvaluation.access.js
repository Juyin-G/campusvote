...
export const assertRubricOpenForResponse = (fair) => {
  ...
  // (starts_at), no al abrirse la inscripción.
  if (!hasFairStarted(fair)) {
    throw ApiError.conflict('Las evaluaciones empiezan cuando inicia la feria');
  }
};

/** Conflicto de interés: nadie califica ni vota un proyecto en el que participa. */
export const assertNoConflictOfInterest = async ({ project, actor, findMember }) => {
  if (project.createdById === actor.id || (await findMember(project.id, actor.id))) {
    throw ApiError.conflict('No puedes evaluar ni votar por un proyecto en el que participas');
  }
};
