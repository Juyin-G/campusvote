...
// Tarjeta de la lista del jurado: la app de iOS muestra portada, categoría y
// stand sin pedir el detalle de cada proyecto.
...
export const listApprovedProjects = async ({ fairId, actor, filters = {} }) => {
  ...
  // Si es JURY, solo sus categorías asignadas. Si además pidió una categoría,
  // se respeta ese filtro dentro de su alcance (antes lo pisaba y la app
  // mostraba todas las categorías del jurado al filtrar por una).
  if (juryCategoryIds !== null) {
    where.categoryId = filters.category_id
      ? { in: juryCategoryIds.filter((id) => id === filters.category_id) }
      : { in: juryCategoryIds };
  }
