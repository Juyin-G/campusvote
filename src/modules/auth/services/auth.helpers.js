...
/**
 * Genera un refresh token opaco (no JWT) y su hash SHA-256 para guardar en BD.
 * Se devuelve el token con los dos nombres: el login y el 2FA leen `rawToken`
 * y el código nuevo lee `token`.
 */
export const generateRefreshToken = () => {
  const token = crypto.randomBytes(40).toString('hex');
  const tokenHash = hashToken(token);
  return { token, rawToken: token, tokenHash };
};

/** Hash de un refresh token para buscarlo en BD. */
