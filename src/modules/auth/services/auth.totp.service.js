...
const log2FASuccess = async (userId, method) => {
  try {
    ...
    logger.warn('No se pudo registrar VERIFY_2FA en auditoría', {
      error: error.message,
    });
  }
};
...
export const setupTotp = async (userId) => {
  ...
  if (user.twoFactorEnabled) {
    throw ApiError.conflict(
      OTP_CONSTANTS.MESSAGES.OTP_ALREADY_ENABLED,
    );
  }

  const secret = otpUtil.generateTotpSecret();

  const uri = otpUtil.generateTotpUri(
    secret,
    user.email,
    user.username,
  );
...
export const verifyAndEnableTotp = async (userId, totpCode) => {
  ...
  const isValid = otpUtil.verifyTotp(
    totpCode,
  ...
  if (!isValid) {
    throw ApiError.badRequest(
      OTP_CONSTANTS.MESSAGES.OTP_CODE_INVALID,
    );
  }
 ...
 */
export const verifyBackupCodeLogin = async (
  userId,
  backupCode,
  meta = {},
) => {
  ...
  if (!user?.twoFactorEnabled) {
    throw ApiError.badRequest(
      OTP_CONSTANTS.MESSAGES.OTP_NOT_ENABLED,
    );
  }
  ...
  if (!valid) {
    throw ApiError.badRequest(
      OTP_CONSTANTS.MESSAGES.BACKUP_CODE_INVALID,
    );
  }
  ...
  await otpRepository.updateBackupCodes(
    userId,
    updatedCodes,
  );
...
/**
 * Manejador unificado de 2FA para login.
 */
export const verifyLoginTotp = async (
  ...
  if (!code) {
    throw ApiError.badRequest(
      'Debes proporcionar un código TOTP o un código de respaldo',
    );
  }
...
export const disableTotp = async (userId, password) => {
  ...
  if (!password) {
    throw ApiError.badRequest(
      'Se requiere la contraseña para deshabilitar 2FA',
    );
  }
  ...
  return {
    message: OTP_CONSTANTS.MESSAGES.OTP_DISABLED,
  };
};
...
export const getTwoFactorStatus = async (userId) => {
  ...
  const backupCodes = Array.isArray(
    user.twoFactorBackupCodes,
  )
    ? user.twoFactorBackupCodes
    : [];
