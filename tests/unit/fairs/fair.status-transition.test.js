...
jest.unstable_mockModule('../../../src/database/prisma.js', () => ({
  prisma: {
    ...
    // Validaciones al abrir la feria (ad55320): sin categorías ni proyectos
    // pendientes de categoría, la transición depende solo del estado.
    fairCategory: { findMany: jest.fn().mockResolvedValue([]) },
    project: { count: jest.fn().mockResolvedValue(0) },
    fairJuryCategoryAssignment: { count: jest.fn().mockResolvedValue(0) },
  },
}));
