...
async function setup() {
  ...
  await prisma.fairJuryAssignment.create({ data: { fairId: fairOpenId, userId: jury.id, assignedById: admin.id } });
  await prisma.fairJuryAssignment.create({ data: { fairId: fairClosedId, userId: juryClosed.id, assignedById: admin.id } });

  projectAId = (await prisma.project.create({ data: { organizationId: orgA.id, fairId: fairOpenId, createdById: student.id, name: 'A', status: 'APPROVED' } })).id;
  projectBId = (await prisma.project.create({ data: { organizationId: orgA.id, fairId: fairOpenId, createdById: student.id, name: 'B', status: 'APPROVED' } })).id;
  await prisma.project.create({ data: { organizationId: orgA.id, fairId: fairClosedId, createdById: admin.id, name: 'ClosedProj', status: 'APPROVED' } });
...
describe('FairVoting — Integration', () => {
  ...
  describe('Concurrencia', () => {
    it('12. Dos requests simultáneos solo registran un voto', async () => {
      ...
      await prisma.fairJuryAssignment.create({ data: { fairId: newFair, userId: j3.id, assignedById: admin.id } });
      const proj = (await prisma.project.create({ data: { organizationId: orgAId, fairId: newFair, createdById: student.id, name: 'Race', status: 'APPROVED' } })).id;
  ...
  describe('Anonimato y resultados', () => {
    ...
    it('17. Los resultados cuentan correctamente los votos', async () => {
      ...
      await prisma.fairJuryAssignment.create({ data: { fairId: fairOpenId, userId: jury2.id, assignedById: admin.id } });
