...
describe('FairResults — publicación oficial', () => {
  beforeAll(async () => {
    ...
    await prisma.project.create({ data: { id: P2, fairId: fairClosedId, organizationId: orgA.id, createdById: student.id, name: 'Bravo', status: 'APPROVED' } });
    castAnonymousVote({ fairId: fairClosedId, projectId: P2 });
    ...
    await prisma.project.create({ data: { id: CLOSED2_P1, fairId: fairClosed2Id, organizationId: orgA.id, createdById: student.id, name: 'PubProj', status: 'APPROVED' } });
