--src/database/sql/challenges/001_candidacy_challenges.sql
...
BEGIN
    ...
    -- Retorno temprano si el payload viene vacío
    ...
    WITH input_data AS (
        -- Deduplicación de entrada para evitar conflictos en ON CONFLICT / UPDATE
        SELECT DISTINCT ON (s->>'institutional_id')
    ...
    stats AS (
        SELECT
            ...
            (SELECT COUNT(*)::INT FROM updated_users) AS dummy_sync_users -- Garantiza ejecución del CTE
    )
    SELECT
        total_processed,
        total_updated,
