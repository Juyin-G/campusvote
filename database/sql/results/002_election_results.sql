-- src/database/sql/results/002_election_results.sql
...
CREATE TABLE IF NOT EXISTS election_results (
    ...
    CONSTRAINT chk_results_counts_non_negative CHECK (
        total_voters >= 0 AND total_votes_cast >= 0 AND blank_votes >= 0 AND null_votes >= 0
    ),
    CONSTRAINT chk_results_votes_lte_voters CHECK (total_votes_cast <= total_voters),
    CONSTRAINT chk_results_blank_null_logic CHECK ((blank_votes + null_votes) <= total_votes_cast),
